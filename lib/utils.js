...
exports.fdate = function(format) {// 格式化时间
	var format = typeof format === 'undefined' ? false : format.toLowerCase(), now = new Date(), time = now.getFullYear() + "-" + (now.getMonth() + 1) + "-"
			+ now.getDate();
...
var resMsg = exports.resMsg = function() {
	...
	return function(code, msg, req) {
		var str = '<html><head><title>' + code + ' ' + msg + '</title></head><body bgcolor="white"><center><h1>' + code + ' ' + msg
				+ '</h1></center><hr><center><p>wants frameword / v ' + _wants_config.version
